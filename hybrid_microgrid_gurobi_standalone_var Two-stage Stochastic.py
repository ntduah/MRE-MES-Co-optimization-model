...
#this is to implement two-stage stochastic model
@author: yangc
'''
import openpyxl
import xlsxwriter
...
global Random_range, Scenario_num, Prob
Random_range = 0.25                                                                                                                                                                                
Scenario_num = 25
Prob = 1/Scenario_num
...
class Parameter():
    def __init__(self):
        ...
        for s in range(Scenario_num):
            self.HWave[s] = {}
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value * 1.5 
                self.HWave[s][t] = random.uniform(cell_value * (1 - 0.05), cell_value * (1 + 0.05)) 
        ...
        for s in range(Scenario_num):
            ...
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value  * 1.5
                self.VWind[s][t] = random.uniform(cell_value * (1 - 0.05), cell_value * (1 + 0.05))        
        ...
        for s in range(Scenario_num):
            self.solar_irri[s] = {}
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value  
                self.solar_irri[s][t] = random.uniform(cell_value * (1 - 0.05), cell_value * (1 + 0.05))  
        ...
        for s in range(Scenario_num):
            ...
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value  
                self.temp[s][t] = random.uniform(cell_value * (1 - 0.05), cell_value * (1 + 0.05))       
        ...
        for s in range(Scenario_num):
            ...
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value
                self.qD[s][t] = random.uniform(cell_value * (1 - Random_range), cell_value * (1 + Random_range))           
        ...
        for s in range(Scenario_num):
            ...
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value  
                self.hD[s][t] = random.uniform(cell_value * (1 - Random_range), cell_value * (1 + Random_range))   
        ...
        for s in range(Scenario_num):
            ...
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value  
                self.eD[s][t] = random.uniform(cell_value * (1 - Random_range), cell_value * (1 + Random_range))   
        ...
        for s in range(Scenario_num):
            ...
            for t in range(self.Time_period):
                cell_value = sheet.cell(row=t+2, column=2).value 
                self.wD[s][t] = random.uniform(cell_value * (1 - Random_range), cell_value * (1 + Random_range))   
...
if __name__ == '__main__':
    ...
    # workbook0 = xlsxwriter.Workbook('MES_result_gurobi_standalone__unlimit_PV_hydro_water_stor_25%RERandom.xlsx')
    workbook0 = xlsxwriter.Workbook('two-stage_stochastic_25scenarios_25%demandchange_unlimitedv5.xlsx')
    ...
    p_Stherm.Start = 0
    v_Shydro.Start = 4814
    v_Swater.Start = 13400



    b_x, q_x, w_x, h_x = {}, {}, {}, {}
    for s in range(Scenario_num):
        b_x[s], q_x[s], w_x[s], h_x[s] = {}, {}, {}, {}
        for t in range(para.Time_period):
            b_x[s][t] = model.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name=f"b_x_{s}_{t}")  
            q_x[s][t] = model.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name=f"q_x_{s}_{t}") 
            w_x[s][t] = model.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name=f"w_x_{s}_{t}") 
            h_x[s][t] = model.addVar(lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name=f"h_x_{s}_{t}") 

    rPow, wFlow, eP_des, q1GT, q2GT, pGT, gFlowP, gFlowB, hFlow, eP_eltz = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
    pWind, pSolar, dV, pWave, ds_water = {},{},{},{},{}
    for s in range(Scenario_num):
        rPow[s], wFlow[s], eP_des[s], q1GT[s], q2GT[s], pGT[s], gFlowP[s], gFlowB[s], hFlow[s], eP_eltz[s] = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
        pWind[s], pSolar[s], dV[s], pWave[s], ds_water[s] = {},{},{},{},{}       
        for t in range(para.Time_period):
            rPow[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"rPow_{s}_{t}")
            
            # Variables for wind turbine
            pWind[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"pWind_{s}_{t}")
            
            # Variables for solar power
            pSolar[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"pSolar_{s}_{t}")
            
            # Variables for wave converter
            dV[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"dV_{s}_{t}")
            pWave[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"pWave_{s}_{t}")
            
            # Variables for water desalination
            wFlow[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"wFlow_{s}_{t}")
            eP_des[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"eP_des_{s}_{t}")
            ds_water[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"ds_water_{s}_{t}")

            # Variables for CHP system
            pGT[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"pGT_{s}_{t}")
            q1GT[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"q1GT_{s}_{t}")
            q2GT[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"q2GT_{s}_{t}")
            gFlowP[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"gFlowP_{s}_{t}")
            gFlowB[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"gFlowB_{s}_{t}")
            
            # Variables for electrolyzer
            hFlow[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"hFlow_{s}_{t}")
            eP_eltz[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"eP_eltz_{s}_{t}")
        
    b_xd, b_xc, q_xd, q_xc, w_xd, w_xc, h_xd, h_xc =   {}, {}, {}, {}, {}, {}, {}, {}
    sB,sQ,sW,sH = {}, {}, {}, {}
    for s in range(Scenario_num):
        b_xd[s], b_xc[s], q_xd[s], q_xc[s], w_xd[s], w_xc[s], h_xd[s], h_xc[s] =   {}, {}, {}, {}, {}, {}, {}, {}
        sB[s],sQ[s],sW[s],sH[s] = {}, {}, {}, {}        
        for t in range(para.Time_period):
            b_xd[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"b_xd_{s}_{t}")  # 'power discharged by battery'
            b_xc[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"b_xc_{s}_{t}")  # 'power charged to battery'
            q_xd[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"q_xd_{s}_{t}")  # 'thermal discharged by thermal storage'
            q_xc[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"q_xc_{s}_{t}")  # 'thermal charged to thermal storage'
            w_xd[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"w_xd_{s}_{t}")  # 'water discharged by water tank'
            w_xc[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"w_xc_{s}_{t}")  # 'water charged to water tank'
            h_xd[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"h_xd_{s}_{t}")  # 'hydrogen discharged by hydrogen tank'
            h_xc[s][t] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"h_xc_{s}_{t}")  # 'hydrogen charged to hydrogen tank'
            sB[s][t] = model.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS, name=f"sB_{s}_{t}")  # 'state of charge, battery'
            sQ[s][t] = model.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS, name=f"sQ_{s}_{t}")  # 'state of charge, thermal storage'
            sW[s][t] = model.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS, name=f"sW_{s}_{t}")  # 'state of charge, water tank'
            sH[s][t] = model.addVar(lb=0, ub=1, vtype=GRB.CONTINUOUS, name=f"sH_{s}_{t}")  # 'state of charge, hydrogen tank'
    ...
    model.addConstr(Cost_oper == gp.quicksum(Prob*(gFlowP[s][t] + gFlowB[s][t]) * para.gas_price for t in range(para.Time_period) for s in range(Scenario_num)) +
                gp.quicksum(Prob*(gFlowP[s][t] + gFlowB[s][t]) * para.carbon_price * para.gas_emmi_factor for t in range(para.Time_period) for s in range(Scenario_num))) 
...
#                  quicksum(para.gamma_B * abs(b_x[t]) + para.gamma_Q * abs(q_x[t]) + para.gamma_H * abs(h_x[t]) + para.gamma_W * abs(w_x[t]) for t in range(para.Time_period)))
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(rPow[s][t] == pWind[s][t] + pSolar[s][t] + pWave[s][t]) 
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            if (para.VWind[s][t] < para.I_w) or (para.VWind[s][t] >= para.Phi_w):
                model.addConstr(pWind[s][t] == 0)
            elif (para.VWind[s][t] >= para.I_w) and (para.VWind[s][t] < para.Lambda_w):
                model.addConstr(pWind[s][t] == N_wind * para.Rated_w * ((para.VWind[s][t] - para.I_w)/ (para.Lambda_w - para.I_w)))
            elif (para.VWind[s][t] >= para.Lambda_w) and (para.VWind[s][t] <= para.Phi_w):
                model.addConstr(pWind[s][t] == N_wind * para.Rated_w)
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(pSolar[s][t] == para.solar_irri[s][t] * para.tal_PV * para.eta_PV * A_pv * (1 - para.gamma_PV * (para.temp[s][t] - 25)))
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(pWave[s][t] == N_wave * para.eta_wave * 0.000001 * para.water_density * para.gravity_accel * para.gravity_accel * para.wave_time * para.HWave[s][t] * para.HWave[s][t] * para.water_front/(32 * math.pi))
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(eP_des[s][t] * 1000 == ds_water[s][t] * wFlow[s][t])
            model.addConstr(ds_water[s][t] == 2.05 * 0.00001 * para.C0 * ((2-para.eta_des)/(2*(1-para.eta_des))) + 2.78 * 0.0000001 * para.mem_resist * wFlow[s][t])
            model.addConstr(wFlow[s][t] <= para.Water_flow_bound * N_osmosis)
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(pGT[s][t] * para.DeltaT == gFlowP[s][t] * para.Vgas * para.eta_P)
            model.addConstr(q1GT[s][t] <= para.eta_X * pGT[s][t] * (1 - para.eta_P - para.eta_L)/para.eta_P)
            model.addConstr(q2GT[s][t] * para.DeltaT <= gFlowB[s][t] * para.Vgas * para.eta_Q)
            model.addConstr(pGT[s][t] <= p_turbine)
            model.addConstr(q2GT[s][t] <= p_boiler)
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):   
            model.addConstr(hFlow[s][t] == para.eta_eltz * eP_eltz[s][t] * para.PH * N_electzer)
            model.addConstr(para.alpha_lb * para.rated_electzer <= eP_eltz[s][t])
            model.addConstr(eP_eltz[s][t] <= para.rated_electzer)
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period): 
            model.addConstr(b_x[s][t] == b_xd[s][t] - b_xc[s][t])    
    
    for s in range(Scenario_num):
        model.addConstr(sB[s][0] * p_Sbatt == para.IB * p_Sbatt - ((b_xd[s][0]/para.eta_bd)) * para.DeltaT + ((b_xc[s][0] * para.eta_bc)) * para.DeltaT)

    for s in range(Scenario_num):
        for t in range(1,para.Time_period):
            model.addConstr(sB[s][t] * p_Sbatt == sB[s][t-1] * p_Sbatt - ((b_xd[s][t]/para.eta_bd)) * para.DeltaT + ((b_xc[s][t] * para.eta_bc)) * para.DeltaT)
            # if t == para.Time_period - 1:
            #     model.addCons(sB[t] >= 0.2)
            if t in para.end_day_time_list:
                model.addConstr(sB[s][t] >= para.IB)

    for s in range(Scenario_num):    
        for t in range(para.Time_period):
            model.addConstr(b_x[s][t] <= p_Sbatt * para.cdr)
            model.addConstr(b_x[s][t] >= -p_Sbatt * para.cdr)
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period): 
            model.addConstr(q_x[s][t] == q_xd[s][t] - q_xc[s][t])    
    
    for s in range(Scenario_num):
        model.addConstr(sQ[s][0] * p_Stherm == para.IQ * p_Stherm - ((q_xd[s][0]/para.eta_qd)) * para.DeltaT + ((q_xc[s][0] * para.eta_qc)) * para.DeltaT)       
    
    for s in range(Scenario_num):
        for t in range(1,para.Time_period):
            model.addConstr(sQ[s][t] * p_Stherm == sQ[s][t-1] * p_Stherm - ((q_xd[s][t]/para.eta_qd)) * para.DeltaT + ((q_xc[s][t] * para.eta_qc)) * para.DeltaT)
            # if t == para.Time_period - 1:
            #     model.addCons(sQ[t] >= 0.2)
            if t in para.end_day_time_list:
                model.addConstr(sQ[s][t] >= para.IQ)
    
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(q_x[s][t] <= p_Stherm * para.cdr)
            model.addConstr(q_x[s][t] >= -p_Stherm * para.cdr) 
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period): 
            model.addConstr(w_x[s][t] == w_xd[s][t] - w_xc[s][t])    
    
    for s in range(Scenario_num):
        model.addConstr(sW[s][0] * v_Swater == para.IW * v_Swater - ((w_xd[s][0]/para.eta_wd)) * para.DeltaT + ((w_xc[s][0] * para.eta_wc)) * para.DeltaT)

    for s in range(Scenario_num):
        for t in range(1,para.Time_period):
            model.addConstr(sW[s][t] * v_Swater == sW[s][t-1] * v_Swater - ((w_xd[s][t]/para.eta_wd)) * para.DeltaT + ((w_xc[s][t] * para.eta_wc)) * para.DeltaT)
            ...
            #initial charge
            if t in para.end_day_time_list:
                model.addConstr(sW[s][t] >= para.IW)
    
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(w_x[s][t] <= v_Swater * para.cdr)
            model.addConstr(w_x[s][t] >= -v_Swater * para.cdr) 
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period): 
            model.addConstr(h_x[s][t] == h_xd[s][t] - h_xc[s][t])    
    
    for s in range(Scenario_num):
        model.addConstr(sH[s][0] * v_Shydro == para.IH * v_Shydro - ((h_xd[s][0]/para.eta_hd)) * para.DeltaT + ((h_xc[s][0] * para.eta_hc)) * para.DeltaT)

    for s in range(Scenario_num):
        for t in range(1,para.Time_period):
            model.addConstr(sH[s][t] * v_Shydro == sH[s][t-1] * v_Shydro - ((h_xd[s][t]/para.eta_hd)) * para.DeltaT + ((h_xc[s][t] * para.eta_hc)) * para.DeltaT)
            ...
            if t in para.end_day_time_list:
                model.addConstr(sH[s][t] >= para.IH)
    
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(h_x[s][t] <= v_Shydro * para.cdr)
            model.addConstr(h_x[s][t] >= -v_Shydro * para.cdr) 
    ...
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(wFlow[s][t] + w_x[s][t]  - para.wD[s][t] == 0)
        
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(q1GT[s][t] + q2GT[s][t] + q_x[s][t]  - para.qD[s][t] == 0)

    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(hFlow[s][t] + h_x[s][t]  - para.hD[s][t] == 0)
    
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            model.addConstr(rPow[s][t] + pGT[s][t] + b_x[s][t]  - eP_des[s][t] - eP_eltz[s][t] - para.eD[s][t] == 0)
    ...
    worksheet0_0.write(0,0, obj_value.X)
    for s in range(Scenario_num):
        for t in range(para.Time_period):
            worksheet0_1.write(s, t, b_x[s][t].X)
            worksheet0_2.write(s, t, q_x[s][t].X)
            worksheet0_3.write(s, t, w_x[s][t].X)
            worksheet0_4.write(s, t, h_x[s][t].X)
            worksheet0_5.write(s, t, b_xd[s][t].X)
            worksheet0_6.write(s, t, b_xc[s][t].X)
            worksheet0_7.write(s, t, q_xd[s][t].X)
            worksheet0_8.write(s, t, q_xc[s][t].X)
            worksheet0_9.write(s, t, w_xd[s][t].X)
            worksheet0_11.write(s, t, h_xd[s][t].X)
            worksheet0_10.write(s, t, w_xc[s][t].X)
            worksheet0_12.write(s, t, h_xc[s][t].X)   
            worksheet0_13.write(s, t, rPow[s][t].X)
            worksheet0_14.write(s, t, wFlow[s][t].X)
            worksheet0_15.write(s, t, eP_des[s][t].X)
            worksheet0_16.write(s, t, q1GT[s][t].X)
            worksheet0_17.write(s, t, q2GT[s][t].X)
            worksheet0_18.write(s, t, pGT[s][t].X)
            worksheet0_19.write(s, t, gFlowP[s][t].X)
            worksheet0_20.write(s, t, gFlowB[s][t].X)
            worksheet0_21.write(s, t, hFlow[s][t].X)
            worksheet0_22.write(s, t, eP_eltz[s][t].X)   
            worksheet0_23.write(s, t, sB[s][t].X)
            worksheet0_24.write(s, t, sQ[s][t].X)
            worksheet0_25.write(s, t, sW[s][t].X)
            worksheet0_26.write(s, t, sH[s][t].X) 
            worksheet0_56.write(s, t, pWind[s][t].X)
            worksheet0_57.write(s, t, pSolar[s][t].X)
            worksheet0_58.write(s, t, pWave[s][t].X) 
            worksheet0_61.write(s, t, ds_water[s][t].X)
    ...
    worksheet0_27.write(0,0, N_wind.X) 
    worksheet0_28.write(0,0, A_pv.X) 
    worksheet0_29.write(0,0, N_wave.X) 
    worksheet0_30.write(0,0, N_osmosis.X) 
    worksheet0_31.write(0,0, N_electzer.X) 
    worksheet0_32.write(0,0, p_turbine.X) 
    worksheet0_33.write(0,0, p_boiler.X)  

    worksheet0_34.write(0,0, Original_Cap_cost_wind.X) 
    worksheet0_35.write(0,0, Original_Cap_cost_pv.X) 
    worksheet0_36.write(0,0, Original_Cap_cost_wave.X) 
    worksheet0_37.write(0,0, Original_Cap_cost_turbine.X) 
    worksheet0_38.write(0,0, Original_Cap_cost_boiler.X) 
    worksheet0_39.write(0,0, Original_Cap_cost_Sbatt.X) 
    worksheet0_40.write(0,0, Original_Cap_cost_Stherm.X)  
    worksheet0_41.write(0,0, Original_Cap_cost_Swater.X) 
    worksheet0_42.write(0,0, Original_Cap_cost_Shydro.X)       
    worksheet0_43.write(0,0, Original_Cap_cost_electzer.X)   
    worksheet0_44.write(0,0, Original_Cap_cost_desal.X) 
    worksheet0_45.write(0,0, Mtn_cost_wind.X) 
    worksheet0_46.write(0,0, Mtn_cost_pv.X) 
    worksheet0_47.write(0,0, Mtn_cost_wave.X) 
    worksheet0_48.write(0,0, Mtn_cost_turbine.X) 
    worksheet0_49.write(0,0, Mtn_cost_boiler.X) 
    worksheet0_50.write(0,0, Mtn_cost_Sbatt.X) 
    worksheet0_51.write(0,0, Mtn_cost_Stherm.X)  
    worksheet0_52.write(0,0, Mtn_cost_Swater.X) 
    worksheet0_53.write(0,0, Mtn_cost_Shydro.X)       
    worksheet0_54.write(0,0, Mtn_cost_electzer.X)   
    worksheet0_55.write(0,0, Mtn_cost_desal.X)

    worksheet0_59.write(0,0, Annualized_Cost_invest.X)   
    worksheet0_60.write(0,0, Cost_oper.X) 
    worksheet0_66.write(0,0, Cost_mtn.X) 

    worksheet0_62.write(0,0,p_Sbatt.X)
    worksheet0_63.write(0,0,p_Stherm.X)
    worksheet0_64.write(0,0,v_Shydro.X)     
    worksheet0_65.write(0,0,v_Swater.X)      
    worksheet0_71.write(0,0, Annualized_Cap_cost_wind.X) 
    worksheet0_72.write(0,0, Annualized_Cap_cost_pv.X) 
    worksheet0_73.write(0,0, Annualized_Cap_cost_wave.X) 
    worksheet0_74.write(0,0, Annualized_Cap_cost_turbine.X) 
    worksheet0_75.write(0,0, Annualized_Cap_cost_boiler.X) 
    worksheet0_76.write(0,0, Annualized_Cap_cost_Sbatt.X) 
    worksheet0_77.write(0,0, Annualized_Cap_cost_Stherm.X)  
    worksheet0_78.write(0,0, Annualized_Cap_cost_Swater.X) 
    worksheet0_79.write(0,0, Annualized_Cap_cost_Shydro.X)       
    worksheet0_80.write(0,0, Annualized_Cap_cost_electzer.X)   
    worksheet0_81.write(0,0, Annualized_Cap_cost_desal.X)   
    workbook0.close()   
